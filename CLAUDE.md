...
3. `generate_manifest.py` runs twice: pass 1 (no flags) writes `assets/metadata/*.json` + hashes-only manifest; pass 2 (`--store-id`/`--root-hash`, after the capsule commit) fills in the URIs
